...
namespace PusherServer
{
    ...
    public class Pusher : IPusher
    {
        ...
        {
            var request = _factory.Build(PusherMethod.POST, resource, requestBody: body);
            ApplyHeaders(request, headers);
            var response = await _options.RestClient.ExecutePostRawAsync(request).ConfigureAwait(false);
        ...
        {
            var request = _factory.Build(PusherMethod.DELETE, resource, parameters);
            ApplyHeaders(request, headers);
        ...
        private static void ApplyHeaders(IPusherRestRequest request, IDictionary<string, string> headers)
        {
