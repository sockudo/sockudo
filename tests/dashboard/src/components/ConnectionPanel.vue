    ...
    <div class="space-y-6 animate-fade-in">
        <div>
            ...
            <p class="text-sm text-surface-400 mt-1">
                Configure and manage your WebSocket connection
            </p>
        </div>

        <!-- Auth server warning -->
        <div
            v-if="authStatus === 'offline'"
            class="bg-surface-900/80 backdrop-blur-xl border border-amber-500/30 bg-amber-500/5 rounded-2xl p-4 flex items-start gap-3"
        >
            ...
            <div>
                ...
                <p class="text-xs text-surface-400 mt-1">
                    Private and presence channels require it. Start with:
                </p>
                <code
                    class="text-xs font-mono text-amber-400/80 bg-surface-900/60 px-2 py-1 rounded mt-1.5 block w-fit"
                    >bun run dev:server</code
                >
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <!-- Config -->
            <div
                class="bg-surface-900/80 backdrop-blur-xl border border-surface-700/40 rounded-2xl p-5 space-y-4"
            >
                ...
                <div
                    v-if="showAdvanced"
                    class="space-y-3 pt-2 border-t border-surface-700/30"
                >
                    <div>
                        <label class="text-xs text-surface-400 mb-1 block"
                            >Cluster</label
                    ...
                    <div>
                        ...
                        >
                        <select
                            v-model="store.config.wireFormat"
                            class="input-field font-mono"
                        >
                    ...
                    <div>
                        ...
                        >
                        <input
                            v-model="store.config.authEndpoint"
                            class="input-field font-mono"
                        />
                    </div>
                    <div>
                        <label class="text-xs text-surface-400 mb-1 block"
                            >User Auth Endpoint</label
                        >
                        <input
                            v-model="store.config.userAuthEndpoint"
                            class="input-field font-mono"
                        />
                    </div>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input
                            v-model="store.config.useTLS"
                            type="checkbox"
                            class="w-4 h-4 rounded border-surface-600 bg-surface-800 text-brand-500 accent-brand-500"
                        />
                ...
                <div class="flex gap-2 pt-2">
                    ...
                    <button
                        v-else
                        @click="disconnect"
                        class="btn-danger flex items-center gap-2 flex-1"
                    >
            ...
            <div class="space-y-4">
                <div
                    class="bg-surface-900/80 backdrop-blur-xl border border-surface-700/40 rounded-2xl p-5"
                >
                    <h3
                        class="text-sm font-semibold text-surface-200 flex items-center gap-2 mb-4"
                    >
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            >
                            <span class="text-xs text-surface-300"
                                >Sockudo V2 over Pusher v7</span
                            >
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-xs text-surface-500"
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            >
                            <span
                                class="text-xs font-mono text-surface-400 truncate max-w-[200px] block"
                            >
                                {{ store.config.useTLS ? "wss" : "ws" }}://{{
                                    store.config.host
                                }}:{{ store.config.port }}/app/{{
                                    store.config.appKey
                                }}?protocol=7&client=js&version=test&format={{
                                    store.config.wireFormat
                                }}
                            </span>
                        </div>
                                ...
                                <div
                                    :class="[
                                        ...
                                        authStatus === 'online'
                                            ...
                                            : authStatus === 'offline'
                                              ? 'bg-red-400'
                                              : 'bg-surface-500 animate-pulse-dot',
                                    ]"
                                />
                                <span class="text-xs text-surface-300">{{
                                    authStatus
                ...
                <div
                    class="bg-surface-900/80 backdrop-blur-xl border border-surface-700/40 rounded-2xl p-5"
                >
                    <h3 class="text-sm font-semibold text-surface-200 mb-4">
                        Quick Actions
                    </h3>
                ...
                >
                    <h3
                        class="text-sm font-semibold text-surface-200 flex items-center gap-2 mb-3"
                    >
                        <ShieldCheck class="w-4 h-4 text-brand-400" /> Auth
                        Server
                    </h3>
                    <p class="text-xs text-surface-400 leading-relaxed">
                        Handles channel authorization for private/presence
                        channels and user authentication for signin. Uses the
                        Pusher server SDK to generate valid HMAC signatures.
                    </p>
