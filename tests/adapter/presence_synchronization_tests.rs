...
async fn test_presence_member_join_broadcast() {
    ...
    let adapter = HorizontalAdapterBase::<MockTransport>::new(config).await.unwrap();
    ...
    adapter_mut.set_cluster_health(&cluster_config).await.unwrap();
    ...
    assert_eq!(requests.len(), 1, "Should have published one presence join request");
    ...
    assert_eq!(request.request_type, sockudo::adapter::horizontal_adapter::RequestType::PresenceMemberJoined);
}
...
async fn test_presence_member_leave_broadcast() {
    ...
    // Verify the request was published
    let requests = adapter_mut.transport.get_published_requests().await;
    assert_eq!(requests.len(), 1, "Should have published one presence leave request");
    ...
    assert_eq!(request.request_type, sockudo::adapter::horizontal_adapter::RequestType::PresenceMemberLeft);
}
...
async fn test_presence_broadcast_skipped_when_cluster_health_disabled() {
    ...
    let requests = adapter_mut.transport.get_published_requests().await;
    assert_eq!(requests.len(), 0, "Should not publish when cluster health disabled");
}
...
async fn test_presence_local_registry_update_always_happens() {
    ...
    // Verify local registry was updated even with cluster health disabled
    {
        ...
        assert!(registry.contains_key(&adapter_mut.node_id), "Local registry should be updated");
        
        let node_data = &registry[&adapter_mut.node_id];
        assert!(node_data.contains_key("presence-channel"), "Channel should exist in registry");
        
        let channel_data = &node_data["presence-channel"];
        assert!(channel_data.contains_key("socket-789"), "Socket should exist in channel");
    ...
    // Verify local registry was cleaned up
    {
        ...
        if let Some(node_data) = registry.get(&adapter_mut.node_id) {
            if let Some(channel_data) = node_data.get("presence-channel") {
                assert!(!channel_data.contains_key("socket-789"), "Socket should be removed from registry");
            }
        }
    }
}
...
async fn test_multiple_presence_members_same_channel() {
    ...
    let users = vec![
        ("user-1", "socket-1", json!({"name": "Alice", "role": "admin"})),
        ("user-2", "socket-2", json!({"name": "Bob", "role": "user"})),
        ("user-3", "socket-3", json!({"name": "Carol", "role": "moderator"})),
    ];
    ...
    for (user_id, socket_id, user_info) in &users {
        ...
        assert!(result.is_ok(), "Presence join should succeed for {}", user_id);
    }
    ...
    // Verify all members are in local registry
    {
        ...
        for (user_id, socket_id, user_info) in &users {
            assert!(channel_data.contains_key(*socket_id), "Socket {} should exist", socket_id);
            let entry = &channel_data[*socket_id];
    ...
    let requests = adapter_mut.transport.get_published_requests().await;
    assert_eq!(requests.len(), 3, "Should have published 3 presence join requests");
}
...
async fn test_presence_state_consistency_across_join_leave() {
    ...
    // Verify final state
    {
        ...
        let registry = horizontal.cluster_presence_registry.read().await;
        let node_data = &registry[&adapter_mut.node_id];
        let channel_data = &node_data["test-channel"];
        
        // Should only have socket-2 now (socket-1 was removed)
        assert!(!channel_data.contains_key("socket-1"), "Old socket should be removed");
        assert!(channel_data.contains_key("socket-2"), "New socket should be present");
