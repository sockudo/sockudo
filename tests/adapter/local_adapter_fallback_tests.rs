...
async fn test_local_adapter_cluster_health_disabled() {
    ...
    let added = adapter.add_to_channel(app_id, channel, &socket_id).await.unwrap();
    assert!(added, "LocalAdapter should successfully add socket to channel");
    
    // Check if in channel
    let exists = adapter.is_in_channel(app_id, channel, &socket_id).await.unwrap();
    assert!(exists, "Socket should exist in channel");
    
    // Remove from channel
    let removed = adapter.remove_from_channel(app_id, channel, &socket_id).await.unwrap();
    assert!(removed, "LocalAdapter should successfully remove socket from channel");
}
...
async fn test_local_adapter_no_horizontal_interface() {
    ...
    let socket_count = local_adapter.get_channel_socket_count("app", "channel").await;
...
async fn test_local_adapter_presence_operations() {
    ...
    adapter.add_to_channel(app_id, channel, &socket1).await.unwrap();
    adapter.add_to_channel(app_id, channel, &socket2).await.unwrap();
    adapter.add_to_channel(app_id, channel, &socket3).await.unwrap();
    ...
    assert_eq!(members.len(), 0, "Non-presence channel should have no members");
    
    // Remove sockets
    adapter.remove_from_channel(app_id, channel, &socket2).await.unwrap();
    
    let socket_count = adapter.get_channel_socket_count(app_id, channel).await;
    assert_eq!(socket_count, 2, "Channel should have 2 sockets after removal");
}
...
async fn test_local_adapter_fallback_from_failed_distributed_adapter() {
    ...
    fallback_adapter.add_to_channel(app_id, channel, &socket_id).await.unwrap();
    
    let exists = fallback_adapter.is_in_channel(app_id, channel, &socket_id).await.unwrap();
    ...
    let socket_count = fallback_adapter.get_channel_socket_count(app_id, channel).await;
    assert_eq!(socket_count, 1, "Fallback adapter should track local sockets");
}
...
async fn test_local_adapter_app_isolation() {
    ...
    adapter.add_to_channel(app1, channel, &socket1).await.unwrap();
    adapter.add_to_channel(app2, channel, &socket2).await.unwrap();
    ...
    assert!(adapter.is_in_channel(app1, channel, &socket1).await.unwrap());
    assert!(!adapter.is_in_channel(app1, channel, &socket2).await.unwrap());
    assert!(adapter.is_in_channel(app2, channel, &socket2).await.unwrap());
    assert!(!adapter.is_in_channel(app2, channel, &socket1).await.unwrap());
}
...
async fn test_local_adapter_concurrent_operations() {
    ...
    for i in 0..20 {
        ...
        let handle = tokio::spawn(async move {
            ...
            // Add to channel
            adapter_guard.add_to_channel(app_id, channel, &socket).await.unwrap();
            
            // Check if exists
            let exists = adapter_guard.is_in_channel(app_id, channel, &socket).await.unwrap();
            assert!(exists);
            
            // Remove from channel
            adapter_guard.remove_from_channel(app_id, channel, &socket).await.unwrap();
        });
        handles.push(handle);
    }
    ...
    let final_count = {
        let mut adapter_guard = adapter.lock().await;
        adapter_guard.get_channel_socket_count(app_id, channel).await
    };
    
    assert_eq!(final_count, 0, "All sockets should be removed after concurrent operations");
}
...
async fn test_local_adapter_memory_cleanup() {
    ...
    for (i, channel) in channels.iter().enumerate() {
        for j in 0..5 {
            ...
            adapter.add_to_channel(app_id, channel, &socket_id).await.unwrap();
        }
    }
    ...
    for j in 0..5 {
        ...
        adapter.remove_from_channel(app_id, &channels[0], &socket_id).await.unwrap();
    }
...
async fn test_local_adapter_with_custom_buffer() {
    ...
    local_adapter.add_to_channel(app_id, channel, &socket_id).await.unwrap();
    
    let exists = local_adapter.is_in_channel(app_id, channel, &socket_id).await.unwrap();
...
async fn test_local_adapter_error_handling() {
    ...
    let count = adapter.get_channel_socket_count(app_id, "non-existent-channel").await;
    assert_eq!(count, 0, "Non-existent channel should have 0 sockets");
    
    let sockets = adapter.get_channel_sockets(app_id, "non-existent-channel").await.unwrap();
    assert_eq!(sockets.len(), 0, "Non-existent channel should return empty socket set");
    
    let members = adapter.get_channel_members(app_id, "non-existent-channel").await.unwrap();
    assert_eq!(members.len(), 0, "Non-existent channel should return empty members");
}
...
async fn test_local_adapter_large_scale() {
    ...
    for c in 0..num_channels {
        let channel = format!("channel-{}", c);
        let count = adapter.get_channel_socket_count(app_id, &channel).await;
        assert_eq!(count, sockets_per_channel, "Channel {} should have {} sockets", c, sockets_per_channel);
    }
    
    // Get channels with socket counts
    let channels_with_counts = adapter.get_channels_with_socket_count(app_id).await.unwrap();
    assert_eq!(channels_with_counts.len(), num_channels, "Should have {} channels", num_channels);
    
    for item in channels_with_counts.iter() {
        let count = item.value();
        assert_eq!(*count, sockets_per_channel, "Each channel should have {} sockets", sockets_per_channel);
    }
    ...
    for c in 0..num_channels {
        ...
        for s in 0..(sockets_per_channel / 2) {
            let socket_id = SocketId(format!("socket-{}-{}", c, s));
            adapter.remove_from_channel(app_id, &channel, &socket_id).await.unwrap();
        }
    }
    ...
    for c in 0..num_channels {
        let channel = format!("channel-{}", c);
        let count = adapter.get_channel_socket_count(app_id, &channel).await;
        assert_eq!(count, sockets_per_channel / 2, "Channel {} should have {} sockets after removal", c, sockets_per_channel / 2);
    }
}