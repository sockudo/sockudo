...
async fn test_request_response_aggregation_logic() {
    ...
    assert!(combined_response.exists); // true if any node returned true
...
async fn test_channel_members_aggregation_deduplication() {
    ...
    members1.insert("user-1".to_string(), sockudo::channel::PresenceMemberInfo {
        user_id: "user-1".to_string(),
        user_info: Some(serde_json::json!({"name": "Alice", "node": "node-1"})),
    });
    members1.insert("user-2".to_string(), sockudo::channel::PresenceMemberInfo {
    ...
    members2.insert("user-1".to_string(), sockudo::channel::PresenceMemberInfo {
        user_id: "user-1".to_string(),
        user_info: Some(serde_json::json!({"name": "Alice", "node": "node-2"})), // Different data
    });
    members2.insert("user-3".to_string(), sockudo::channel::PresenceMemberInfo {
...
async fn test_channels_with_socket_count_aggregation() {
    ...
    assert_eq!(combined_response.channels_with_sockets_count["channel-A"], 5); // 2 + 3
    assert_eq!(combined_response.channels_with_sockets_count["channel-B"], 1); // Only from node-1
    assert_eq!(combined_response.channels_with_sockets_count["channel-C"], 1); // Only from node-2
}
...
async fn test_exists_flag_aggregation_logic() {
    ...
    assert!(!combined_response.exists, "Should be false when no node returns true");
    ...
    assert!(combined_response.exists, "Should be true when any node returns true");
}
...
async fn test_request_timeout_handling() {
    ...
    // Send a request that will timeout (no other nodes to respond)
    ...
    assert!(elapsed.as_millis() >= 100, "Should wait at least the timeout duration");
    assert!(elapsed.as_millis() < 500, "Should not wait much longer than timeout");
    
    // Response should indicate no results (empty response)
