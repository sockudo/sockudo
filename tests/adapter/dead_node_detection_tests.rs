...
async fn test_heartbeat_tracking_updates_node_registry() {
    ...
    // Verify the node is now tracked in heartbeat registry
    {
        ...
        assert!(heartbeats.contains_key("remote-node-1"), "Remote node should be tracked after heartbeat");
        assert!(heartbeats.len() == 1, "Should have exactly 1 remote node tracked");
    }
}
...
async fn test_dead_node_detection_identifies_timeout_nodes() {
    ...
    assert_eq!(dead_nodes[0], "old-node", "Should identify the old node as dead");
}
...
async fn test_cleanup_leader_election_excludes_dead_nodes() {
    ...
    assert!(is_leader, "This node should become leader when the natural leader is dead");
}
...
async fn test_dead_node_cleanup_removes_presence_data() {
    ...
    // Verify presence data exists before cleanup
    {
        ...
        assert!(registry.contains_key(dead_node_id), "Dead node should have presence data before cleanup");
        assert_eq!(registry[dead_node_id].len(), 2, "Dead node should have 2 channels of presence data");
    }
    
    // Perform dead node cleanup
    let cleanup_tasks = {
        let horizontal = adapter.horizontal.lock().await;
        horizontal.handle_dead_node_cleanup(dead_node_id).await.unwrap()
    };
    
    // Verify cleanup tasks were generated
    assert_eq!(cleanup_tasks.len(), 2, "Should generate 2 cleanup tasks (one per unique user)");
    
    // Verify presence data was removed
    {
        let horizontal = adapter.horizontal.lock().await;
        let registry = horizontal.cluster_presence_registry.read().await;
        assert!(!registry.contains_key(dead_node_id), "Dead node should have no presence data after cleanup");
    }
...
async fn test_dead_node_event_structure_contains_required_data() {
    ...
    let mut event_receiver = adapter.configure_dead_node_events()
...
async fn test_follower_cleanup_only_removes_registry_data() {
    ...
    // Process the dead node notification
    {
        ...
        assert!(result.is_ok(), "Dead node notification processing should succeed");
    }
    ...
    // Verify both heartbeat and presence registry were cleaned up
    {
        ...
        let heartbeats = horizontal.node_heartbeats.read().await;
        assert!(!heartbeats.contains_key(dead_node_id), "Dead node should be removed from heartbeat tracking");
        
        // Check presence registry
        let registry = horizontal.cluster_presence_registry.read().await;
        assert!(!registry.contains_key(dead_node_id), "Dead node should be removed from presence registry");
    }
}