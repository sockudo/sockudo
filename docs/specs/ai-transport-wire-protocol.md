...
Connection parameters: `append_rollup_window=<ms>` and `append_mode=delta|full`. Per-app config clamps allowed rollup values. `0` disables rollup. `append_mode=delta` is the V2 default and delivers live `sockudo:message.append` frames with only the appended fragment in `data`, matching Ably-style token delivery. `append_mode=full` delivers the accumulated message snapshot on each live append. Persistence, version history, late-join rewind, recovery, webhooks, and push continue to use every original mutation and the latest aggregate, independent of append delivery mode. Current server v1 validates the rollup connection parameter but uses `[ai_transport.rollup].default_window_ms` for node-local channel fanout. Rate limiting counts original mutation requests before rollup; egress metrics count coalesced deliveries. Rollup output must reduce to the same mutable message state as the unrolled append stream.
