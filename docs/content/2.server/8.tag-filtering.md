...
::callout{type="info"}
Tag filtering is a **protocol V2 only** feature. V1 (Pusher-compatible) connections receive all messages without server-side filtering. V2 connections support tag-based filtering with serial numbers and message IDs.
::
...
Configure in `config/config.toml` (or `config/config.json`):
...
const btcChannel = client.subscribe(
  "market-data",
  Filter.eq("symbol", "BTC")
);

// Subscribe to large trades (any symbol)
const largeTradesChannel = client.subscribe(
  "market-data",
  Filter.gt("volume", "5000")
);

// Subscribe to crypto on specific exchanges
const cryptoExchangeChannel = client.subscribe(
  "market-data",
  Filter.and(
    Filter.eq("type", "crypto"),
...
const tempSensors = client.subscribe(
  "iot-sensors",
  Filter.and(
...
const criticalAlerts = client.subscribe(
  "iot-sensors",
  Filter.and(
...
const zoneMonitoring = client.subscribe(
  "iot-sensors",
...
const urgent = client.subscribe(
  "notifications",
  Filter.eq("priority", "high")
);

// Notifications for specific user
const userNotifications = client.subscribe(
  "notifications",
  Filter.eq("user_id", currentUserId)
);

// Important OR mentions
const importantOrMentions = client.subscribe(
  "notifications",
  Filter.or(
...
const matchEvents = client.subscribe(
  "game-events",
  Filter.eq("match_id", "12345")
);

// Events for player's region
const regionalEvents = client.subscribe(
  "game-events",
  Filter.and(
...
const lootDrops = client.subscribe(
  "game-events",
  Filter.and(
...
const channel = client.subscribe("test-channel", testFilter);
