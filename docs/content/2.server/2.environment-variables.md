...
- Apache Iggy queue uses `IGGY_*`

There is not a second `QUEUE_NATS_*` / `QUEUE_RABBITMQ_*` / `QUEUE_KAFKA_*` / `QUEUE_PULSAR_*` / `QUEUE_GOOGLE_PUBSUB_*` / `QUEUE_IGGY_*` namespace yet.
...
| `IGGY_USERNAME` | `string` | — | Optional username override |
| `IGGY_PASSWORD` | `string` | — | Optional password override |
...
| `IGGY_PARTITIONS_COUNT` | `u32` | `1` | Topic partitions to create |
| `IGGY_PARTITION_ID` | `u32` | `1` | Adapter publish/poll partition |
