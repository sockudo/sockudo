...
| Variable | Type | Default | Description |
|---|---|---|---|
| `ADAPTER_DRIVER` | `string` | `local` | Pub/sub adapter: `local`, `redis`, `redis-cluster`, `nats`, `rabbitmq`, `google-pubsub`, `kafka`, `iggy` |
| `CACHE_DRIVER` | `string` | `memory` | Cache backend: `memory`, `redis`, `redis-cluster`, `none` |
| `QUEUE_DRIVER` | `string` | `redis` | Queue backend: `memory`, `redis`, `redis-cluster`, `nats`, `rabbitmq`, `kafka`, `pulsar`, `google-pubsub`, `sqs`, `sns`, `iggy`, `none` |
...
| `ADAPTER_FALLBACK_TO_LOCAL` | `bool` | `true` | Fall back to local adapter on initialization failure |
...
| Variable | Type | Default | Description |
|---|---|---|---|
| `VERSIONED_MESSAGES_ENABLED` | `bool` | `false` | Enable V2 mutable-message retrieval and mutation endpoints |
| `VERSIONED_MESSAGES_DRIVER` | `string` | `memory` | Version-store backend: `memory`, `postgres`, `mysql`, `dynamodb`, `surrealdb`, `scylladb` |
| `VERSIONED_MESSAGES_MAX_PAGE_SIZE` | `usize` | `100` | Maximum server-side page size for version-history reads |
| `VERSIONED_MESSAGES_RETENTION_WINDOW_SECONDS` | `u64` | `0` | How long (seconds) to retain message versions; `0` = retain forever |
| `VERSIONED_MESSAGES_PURGE_INTERVAL_SECONDS` | `u64` | `300` | How often the background purge task runs |
| `VERSIONED_MESSAGES_PURGE_BATCH_SIZE` | `usize` | `1000` | Number of versions processed per purge batch |
| `VERSIONED_MESSAGES_MAX_PURGE_PER_TICK` | `usize` | `100000` | Maximum versions purged in a single tick |

## Channel History

| Variable | Type | Default | Description |
|---|---|---|---|
| `HISTORY_ENABLED` | `bool` | `false` | Enable durable channel history |
| `HISTORY_REWIND_ENABLED` | `bool` | `true` | Allow clients to rewind and replay history on subscribe |
| `HISTORY_BACKEND` | `string` | `postgres` | History storage backend: `memory`, `postgres`, `mysql`, `dynamodb`, `surrealdb`, `scylladb` |
| `HISTORY_RETENTION_WINDOW_SECONDS` | `u64` | `86400` | How long (seconds) to retain history entries; also used as the purge cutoff |
| `HISTORY_MAX_PAGE_SIZE` | `usize` | `100` | Maximum items returned per history page |
| `HISTORY_WRITER_SHARDS` | `usize` | `16` | Number of async writer shards for parallel writes |
| `HISTORY_WRITER_QUEUE_CAPACITY` | `usize` | `4096` | Capacity of each writer shard's internal queue |
| `HISTORY_MAX_MESSAGES_PER_CHANNEL` | `usize` | *(unset)* | Cap on retained messages per channel (unset = no cap) |
| `HISTORY_MAX_BYTES_PER_CHANNEL` | `u64` | *(unset)* | Cap on retained payload bytes per channel (unset = no cap) |
| `HISTORY_POSTGRES_TABLE_PREFIX` | `string` | `sockudo_history` | Table name prefix for the PostgreSQL backend |
| `HISTORY_POSTGRES_WRITE_TIMEOUT_MS` | `u64` | `5000` | Write timeout in milliseconds for the PostgreSQL backend |
| `HISTORY_PURGE_INTERVAL_SECONDS` | `u64` | `300` | How often the background purge worker runs (PostgreSQL, MySQL, SurrealDB) |
| `HISTORY_PURGE_BATCH_SIZE` | `usize` | `1000` | Number of entries deleted per purge batch |
| `HISTORY_MAX_PURGE_PER_TICK` | `usize` | `100000` | Maximum entries purged in a single tick |
...
### Queue: NATS / RabbitMQ / Kafka / Pulsar / Google Pub/Sub / Apache Iggy

These queue drivers currently reuse the same broker env vars as their adapter counterparts:

- NATS queue uses `NATS_*` and requires JetStream
- RabbitMQ queue uses `RABBITMQ_*`
- Kafka queue uses `KAFKA_*`
- Pulsar queue uses `PULSAR_*`
- Google Pub/Sub queue uses `GOOGLE_PUBSUB_*` and `PUBSUB_EMULATOR_HOST`
- Apache Iggy queue uses `IGGY_*`, with optional `ADAPTER_IGGY_*` and `QUEUE_IGGY_*` partition overrides

There is not a second `QUEUE_NATS_*` / `QUEUE_RABBITMQ_*` / `QUEUE_KAFKA_*` / `QUEUE_PULSAR_*` / `QUEUE_GOOGLE_PUBSUB_*` namespace yet.

### Apache Iggy Adapter / Queue

| Variable | Type | Default | Description |
|---|---|---|---|
| `IGGY_CONNECTION_STRING` | `string` | `iggy://iggy:iggy@127.0.0.1:8090` | Apache Iggy SDK connection string |
| `IGGY_USERNAME` | `string` | — | Optional username when credentials are not embedded in `IGGY_CONNECTION_STRING` |
| `IGGY_PASSWORD` | `string` | — | Optional password when credentials are not embedded in `IGGY_CONNECTION_STRING` |
| `IGGY_CONSUMER_NAME` | `string` | `INSTANCE_PROCESS_ID` or `HOSTNAME` | Stable adapter consumer name seed |
| `IGGY_STREAM` | `string` | `sockudo` | Shared Iggy stream name |
| `IGGY_TOPIC_PREFIX` | `string` | `sockudo-adapter` | Adapter topic prefix |
| `IGGY_QUEUE_TOPIC_PREFIX` | `string` | `sockudo-queue` | Queue topic prefix |
| `IGGY_CONSUMER_GROUP_PREFIX` | `string` | `sockudo-workers` | Queue consumer group prefix |
| `IGGY_REQUEST_TIMEOUT_MS` | `u64` | `5000` | Broker operation timeout in ms |
| `IGGY_POLL_INTERVAL_MS` | `u64` | `50` | Poll loop interval in ms |
| `IGGY_POLL_BATCH_SIZE` | `u32` | `100` | Max messages per poll |
| `IGGY_PARTITIONS_COUNT` | `u32` | `1` | Shared fallback for adapter and queue topic partitions |
| `ADAPTER_IGGY_PARTITIONS_COUNT` | `u32` | `IGGY_PARTITIONS_COUNT` | Adapter topic partitions to create |
| `QUEUE_IGGY_PARTITIONS_COUNT` | `u32` | `IGGY_PARTITIONS_COUNT` | Queue topic partitions to create |
| `IGGY_PARTITION_ID` | `u32` | `0` | Shared compatibility fallback partition |
| `ADAPTER_IGGY_PARTITION_ID` | `u32` | `IGGY_PARTITION_ID` | Adapter fallback partition |
| `QUEUE_IGGY_PARTITION_ID` | `u32` | `IGGY_PARTITION_ID` | Queue fallback partition |
| `IGGY_AUTO_CREATE` | `bool` | `true` | Create missing streams, topics, and queue groups |
| `IGGY_START_FROM_LATEST` | `bool` | `true` | Adapter listeners start from latest retained message |
| `IGGY_NODES_NUMBER` | `u32` | — | Explicit cluster size override when not using cluster health |
...
| `NATS_PREFIX` | `string` | `sockudo_adapter:` | Subject prefix |
...
| `NATS_DISCOVERY_MAX_WAIT_MS` | `u64` | `1000` | Upper bound for best-effort system ping discovery |
| `NATS_DISCOVERY_IDLE_WAIT_MS` | `u64` | `150` | Stop waiting this long after the last discovery reply |
| `NATS_NODES_NUMBER` | `u32` | — | Explicit cluster size override when not using cluster health |
| `NATS_SUBSCRIPTION_CAPACITY` | `usize` | — | Per-subscriber message buffer capacity (unset = use `async-nats` default) |
| `NATS_CLIENT_CAPACITY` | `usize` | — | Outbound publish buffer capacity (unset = use `async-nats` default) |
| `NATS_MAX_RECONNECTS` | `usize` | — | Max reconnect attempts before giving up (unset = unlimited) |

## Pulsar Adapter

| Variable | Type | Default | Description |
|---|---|---|---|
| `PULSAR_URL` | `string` | `pulsar://127.0.0.1:6650` | Pulsar broker URL |
| `PULSAR_PREFIX` | `string` | `sockudo-adapter` | Topic and subscription prefix |
| `PULSAR_TOKEN` | `string` | — | Pulsar token auth |
| `PULSAR_REQUEST_TIMEOUT_MS` | `u64` | `5000` | Request timeout in ms |
| `PULSAR_NODES_NUMBER` | `u32` | — | Explicit cluster size override when not using cluster health |
...
| Variable | Type | Default | Description |
|---|---|---|---|
| `HEALTH_CHECK_TIMEOUT_MS` | `u64` | `400` | Per-subsystem timeout for `/up` checks (adapter, cache, queue, app manager run in parallel). `/live` ignores this and returns immediately. |
