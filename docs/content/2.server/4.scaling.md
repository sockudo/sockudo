...
| `rabbitmq` | Multi-node with RabbitMQ fanout exchanges | RabbitMQ server |
| `google-pubsub` | Managed multi-node on Google Cloud Pub/Sub | GCP Pub/Sub project |
| `kafka` | Multi-node with Kafka topics and consumer groups | Kafka-compatible broker |
| `iggy` | Multi-node with Apache Iggy streams and topics | Apache Iggy server |
...
Uses RabbitMQ fanout exchanges for broadcast, request, and response channels.
...
Uses dedicated topics plus one subscription per Sockudo node for fan-out semantics.
...
GOOGLE_PUBSUB_PROJECT_ID=local-project
```

## Kafka Adapter

Uses three Kafka topics plus a unique consumer group per Sockudo node.
...
Uses one Apache Iggy stream with three topics for broadcast, request, and response traffic. Each Sockudo node uses unique regular consumers so every node receives the horizontal messages it needs.
...
| `topic_prefix` | `string` | `sockudo-adapter` | Topic prefix for broadcast/request/response topics |
...
| `nodes_number` | `number?` | `null` | Expected cluster node count when cluster health discovery is disabled |

Iggy adapter listeners commit offsets after the local handler finishes. Request messages are committed only after the handler succeeds and the response is published. When `start_from_latest` is enabled, offsets are seeded only for consumers without an existing offset record, so restarting a node resumes from the broker-stored offset instead of skipping messages produced during downtime.
...
### Presence History In Clusters

Presence history is written from the same live join/leave decisions that drive membership events, but the durable store is the final duplicate-suppression boundary.

- The authoritative historical transition is the first retained `member_added` or `member_removed` edge for a given `(app, channel, user)` state.
- If multiple nodes independently report the same edge during concurrent joins, reconnect churn, or duplicate inter-node notifications, Sockudo retains one row and suppresses later same-edge duplicates until the opposite edge is recorded.
- Orphan cleanup can record a `member_removed` with cause `orphan_cleanup`, but only when that removal is still the first retained removal edge. Duplicate cleanup notifications are suppressed.
- If a node dies before its durable write happens, live cluster cleanup still removes orphaned members, but the missed historical transition cannot be reconstructed retroactively.
- If a presence-history write fails, later `/presence/history` reads mark continuity as degraded and `complete = false`; live presence membership continues to function.
- If retention has already evicted older transitions, reads report `truncated_by_retention = true` instead of pretending the older slice is complete.

#### Operational Outcomes

- Node restart with durable history enabled: retained presence-history events survive because presence history inherits the same durable backend family as channel history.
- Node restart without durable history enabled: the process-local presence-history stream is lost, a new `stream_id` starts on that node, and previously retained presence-history events from that node are no longer available. Live presence state can repopulate, but retained history before the restart is gone.
- Durable-store outage or write failure before any retained rows exist: the channel moves to `degraded`. Live presence still works, `/presence/history` remains readable, but `continuity.complete = false` and `stream_state.continuity_proven = false`.
- Durable-store outage or write failure after retained rows already exist: the channel moves to `reset_required`. Sockudo keeps the retained rows visible for inspection, but continuity for that stream is no longer trustworthy until operators call `/presence/history/reset`.
- Replay of delayed cleanup messages: duplicate delayed cleanup removals collapse against the current retained user edge. If they arrive after a stream has already been marked `reset_required`, they do not restore continuity; operators still need an explicit reset before the stream is considered healthy again.
- Retention compaction: the channel can stay `healthy` while the retained floor advances. That is exposed as higher `oldest_available_serial` or `oldest_available_published_at_ms` plus `truncated_by_retention = true`, not as degradation.

#### Detecting Problems Early

- Watch `/stats` for `presence_history.degraded_channels > 0` or `presence_history.reset_required_channels > 0`.
- Inspect one channel with `GET /apps/{appId}/channels/{channelName}/presence/history/state` when you need to distinguish retention compaction from a real store incident.
- Exported metrics include retained-event gauges, write-failure counters, queue depth, degraded-channel counts, and reset-required-channel counts, so alerts can trigger before users notice missing historical continuity.
...
| Fallback to local | `ADAPTER_FALLBACK_TO_LOCAL` | `true` | Fall back to local adapter on initialization failure |
...
**Fallback to local** controls what happens when a horizontal adapter fails to initialize (e.g., the backend is unreachable or the required Cargo feature was not compiled in). When enabled, Sockudo logs a warning and falls back to the local adapter. When disabled, the server returns a hard error and refuses to start:

```bash
ADAPTER_FALLBACK_TO_LOCAL=false
```

::callout{type="warning"}
In production multi-node deployments, set `fallback_to_local = false` to prevent a node from silently starting without horizontal scaling.
::
...
| Feature | Local | Redis | Redis Cluster | NATS | RabbitMQ | Google Pub/Sub | Kafka |
|---|---|---|---|---|---|---|---|
| Horizontal scaling | No | Yes | Yes | Yes | Yes | Yes | Yes |
| Auto-reconnection | N/A | Automatic (5 retries) | Manual with backoff | Built-in (NATS lib) | Broker reconnect via client | Managed by GCP client | Broker reconnect via client |
| Node counting | 1 | `PUBSUB NUMSUB` | `PUBSUB NUMSUB` | Manual (`nodes_number`) | Manual (`nodes_number`) | Manual (`nodes_number`) | Manual (`nodes_number`) |
| TLS support | N/A | Via `REDIS_URL` | Via `use_tls` | Via NATS URL scheme | Via AMQP URL/TLS | Google-managed / emulator endpoint | Via broker security config |
| Auth | N/A | Password/ACL | Password/ACL | User/pass or token | User/pass in AMQP URL | ADC or emulator | SASL / broker auth |
| Health check | N/A | `PING` | `PING` | Connection state | Connection state | Config validation | Metadata fetch |
...
| Queue | `memory`, `redis`, `redis-cluster`, `nats`, `rabbitmq`, `kafka`, `pulsar`, `google-pubsub`, `sqs`, `sns`, `iggy`, `none` | Webhook delivery |
...
# Google Pub/Sub adapter
cargo build --features google-pubsub

# Kafka adapter
cargo build --features kafka

# Apache Iggy adapter and queue
cargo build --features iggy

# Multiple backends
cargo build --features "redis,nats,rabbitmq,google-pubsub,kafka,iggy,postgres"
...
```json
{
  "adapter": {
    "driver": "rabbitmq",
    "rabbitmq": {
      "url": "amqp://sockudo:secret@rabbitmq:5672/%2f",
      "nodes_number": 3
    }
  }
}
...
```json
{
  "adapter": {
    "driver": "iggy",
    "iggy": {
      "connection_string": "iggy://iggy:iggy@iggy:8090",
      "stream": "sockudo",
      "topic_prefix": "sockudo-adapter",
      "nodes_number": 3
    }
