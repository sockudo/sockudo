...
ExecStart=/opt/sockudo/sockudo --config /opt/sockudo/config/config.toml
Restart=always
RestartSec=5
...
| `surrealdb` | SurrealDB 3 app manager |
...
| **Probes** | Liveness and startup on `/live`, readiness on `/up` |
...
|---|---|---|
| `/live` | `6001` | Liveness probe — `200 OK` without checking dependencies |
| `/up` | `6001` | Readiness check — `OK` / `DEGRADED` / `ERROR` |
| `/up/{app_id}` | `6001` | Same as `/up` plus validates the app exists and is enabled |
...
- [ ] Configure `policy.channels.allowed_origins` per app to restrict WebSocket connections
