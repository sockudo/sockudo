...
::callout{type="info"}
Delta compression is a **protocol V2 only** feature. V1 (Pusher-compatible) connections receive plain full messages without delta encoding. V2 connections use `sockudo:delta`, `sockudo:delta_cache_sync`, and other delta events with the `sockudo:` prefix, alongside serial numbers and message IDs.
::
...
Enable delta compression globally in `config/config.toml` (or `config/config.json`):
...
app_manager.array.apps.policy.channels.channel_delta_compression."slow-channel" = "disabled"
```
...
Configure in the app's `policy.channels.channel_delta_compression` object:

```toml
[app_manager.array.apps.policy.channels.channel_delta_compression."pattern:*"]
enabled = true
algorithm = "fossil"
...
app_manager.array.apps.policy.channels.channel_delta_compression."disable-this" = "disabled"
```
...
```toml
[app_manager.array.apps.policy.channels.channel_delta_compression."high-frequency:*"]
algorithm = "fossil" # Faster, good enough

[app_manager.array.apps.policy.channels.channel_delta_compression."large-payloads:*"]
...
```json
{
  "event": "sockudo:delta_cache_sync",
  "channel": "market:live",
  "data": {
    "enabled": true,
    "algorithm": "fossil",
...
const channel = client.subscribe("market:live", {
  delta: { enabled: true, algorithm: "fossil" }
});

// Disable delta for this channel
const otherChannel = client.subscribe("no-delta", {
  delta: { enabled: false }
});
```
...
Delta compression is V2-only. V1 clients (pusher-js, Laravel Echo) receive plain full messages without any delta encoding.
