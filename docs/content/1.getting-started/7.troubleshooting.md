...
The response body indicates which subsystem failed (e.g. `Adapter health check timeout`). The per-subsystem timeout is configurable via `HEALTH_CHECK_TIMEOUT_MS`.
