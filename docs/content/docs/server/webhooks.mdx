...
Optional in-process batching admits at most 2,048 jobs and 16 MiB of encoded
jobs plus charged record overhead. A full buffer returns a capacity error to
the producer. Batches flush on size or time; a failed queue transfer retains
the accepted batch and its stable job IDs for retry. Graceful shutdown stops
admission and drains these batches before disconnecting the queue. A dependency
outage may keep draining pending until the server's shutdown deadline; forced
process termination can still lose work that has not reached a durable queue.
