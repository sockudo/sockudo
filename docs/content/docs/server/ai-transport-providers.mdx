...
<div
  ...
  role="img"
  aria-label="Provider flow showing how the user, Sockudo channel, agent worker, model provider, and durable stores cooperate during an AI turn."
>
  ...
  const turn = transport.newTurn({
    ...
    onCancel(request) {
      return (
        ...
        request.turnOwners.get(body.turnId) === body.clientId
      );
    },
  });
...
message stream: Sockudo persists ordered UI chunks, turn lifecycle events, approval responses, and
...
| Tool approvals                            | Persist approval requests and responses as normal turn state. The app still owns approval policy, signature checks, and tool execution authorization.  |
...
| Telemetry and lifecycle callbacks         | Correlate AI SDK spans with `channelName`, `turnId`, and `invocationId`. Sockudo metrics continue to describe transport, history, and fanout health.   |
