...
<div
  className="ai-model-diagram not-prose"
  role="img"
  aria-label="AI Transport end-to-end model from clients and providers through a Sockudo session channel into durable stores and fanout surfaces."
>
...
<div
  className="turn-lifecycle-diagram not-prose"
  role="img"
  aria-label="AI Transport turn lifecycle from client input through Sockudo persistence, agent output, append rollup, and final history state."
>
...
| ------------------------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| Sockudo AI Transport                  | Published user inputs, assistant output chunks, approval responses, turn lifecycle, and cursors.  | Browser refresh, mobile handoff, reconnect, late join, transcript replay, and audit workflows. |
...
or agent checkpoint id in turn metadata, then correlate it with `channelName`, `turnId`, and
...
export async function runTurn({ body, sockudo }) {
  ...
  await turn.streamResponse(toUIMessageStream({ stream: result.stream }));
