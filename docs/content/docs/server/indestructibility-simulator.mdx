...
| Network delay/drop/duplication | Applies seeded publisher-side delay, drop-before-send, and duplicate-send faults. Duplicate sends reuse the same idempotency key. |
| Config changes/reloads | Writes a second config and applies it by process restart because Sockudo loads config at startup. |
| Push-provider fake outcomes | Optionally starts `scripts/push-mock-provider.mjs` and points FCM worker env vars at it when the binary is built with push/monolith features. |
...
| File | Purpose |
| --- | --- |
| `artifact.json` | Seed, command, generated config paths, fault timeline, counters, publish results, and recovery observations. |
...
transition counts, provider results, queue losses, repair counts, recent push trace entries, and
top-level `io_trace` entries for recent deterministic operation/fault decisions. Keep sample
