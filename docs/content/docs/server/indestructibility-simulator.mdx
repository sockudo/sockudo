...
| `DeterministicStorage` | Storage read/write behavior around real memory-store calls: fail-before-commit, fail-after-commit, lost responses, stale reads, and backend outages. |
...
counts, provider results, queue losses, repair counts, recent push trace entries, and top-level
`io_trace` entries for recent deterministic operation/fault decisions. Keep sample outputs with the
exact seed and tick count that produced them so they remain replayable. A compact example generated
with `--seed 12648430 --ticks 16 --json` lives at
