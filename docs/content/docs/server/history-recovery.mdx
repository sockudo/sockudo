...
The native V2 attach gate registers the subscriber before capturing the durable
high-water mark. Concurrent publishes are held in a count-and-byte-bounded gate,
shared across subscribers with immutable message references, then drained after
rewind without a lock held across delivery. Overflow closes the affected
connection because the live-plus-replay sequence can no longer be proven.
