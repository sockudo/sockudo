...
Durable transition checks retain bounded metadata for up to 64 channels per store, with a 256 KiB accounting limit per cached channel. After proving complete stream coverage, the store reads newly appended records and uses conservative summaries of serial ranges to locate older users or duplicate transition keys. A summary can cause extra reads but cannot prove a duplicate by itself. Cold lookups may therefore read durable pages; repeated lookups can reuse verified outcomes. Expired ranges release their summary slots, and a changed stream generation discards cached evidence. Missing records, incomplete pages, or a reset during lookup fail closed.
