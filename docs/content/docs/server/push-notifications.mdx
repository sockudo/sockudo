...
For ActivityKit start, update, end, push-token rotation, and iOS 18 broadcast channels, see
[Apple Live Activities](/docs/server/apple-live-activities).
...
APNs connection pooling, Live Activity topics, broadcast and channel-management endpoints, and
timeouts are configured under `[push.apns]`. Broadcast delivery uses one APNs request per channel,
instead of expanding the audience into device-token jobs.
