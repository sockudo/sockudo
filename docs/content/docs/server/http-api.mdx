...
store plus the native push-store and message services; they do not call Sockudo
HTTP handlers internally. Stats pages use opaque cursors and stable Link query
propagation. Realtime push recipients are delivered as ordinary native
publishes, while external provider delivery continues through the configured
push workers.
