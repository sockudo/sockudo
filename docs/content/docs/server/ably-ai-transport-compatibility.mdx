---
title: Ably REST, WebSocket, and AI Transport compatibility
description: Sockudo's evidence-backed Ably REST and WebSocket compatibility surface, excluding Live Objects.
icon: BadgeCheck
---
...
remain red on fatal browser diagnostics and/or assertions. See the dated
`sockudo-compatibility/findings/2026-07-15-final-verification.md` report before
publishing an unqualified compatibility claim.

## Compatibility Matrix

| Surface | Status | Scope |
| --- | --- | --- |
| Ably Realtime WebSocket JSON/MsgPack protocol | Node-default evidence green; browser gates red | Root WebSocket endpoint accepts Ably `ProtocolMessage` connect, attach, detach, message, presence, auth, heartbeat, ACK, and NACK flows covered by pinned `ably@2.21.0` JSON and MsgPack lanes. Non-WebSocket transports are intentionally unsupported. |
| Ably REST time/token/history/message reads and publish | Node-default evidence green; browser gates red | `/time`, `/keys/{keyName}/requestToken`, `/keys/{keyName}/revokeTokens`, `/messages` batch publish, `/channels/{channel}/messages`, message lookup, version lookup, and presence reads/history exist for the selected SDK path. Full Ably REST API parity is not claimed. |
...
| Mutable messages and output streaming | Node-default evidence green; browser REST gate red | Ably `publish`, `appendMessage`, `updateMessage`, `deleteMessage`, `getMessage`, `getMessageVersions`, and history map to Sockudo versioned messages. |
