...
Dead-letter queue messages remain owned by the configured queue backend. SQL schemas include
`push_dead_letters` for durable inspection records, and cleanup removes expired rows if a backend
has written them, but the current public API does not expose dead-letter listing yet.
...
| Dead letters | `sockudo_push_retry_dead_lettered_total`, dead-letter queue depth |
...
need to inspect queue-native dead-letter details and retry/dead-letter metrics before replaying or
republishing.
