...
| `ADAPTER_SERIALIZATION` | Override `adapter.serialization`. Set to `"msgpack"` to enable MessagePack encoding for inter-node pub/sub payloads. |
...
| `DATABASE_REDIS_MASTER_TLS_ENABLED` | Enables TLS for the data connection to the Sentinel-managed master/replica. |
| `DATABASE_REDIS_MASTER_TLS_ACCEPT_INVALID_CERTS` | Skips certificate/hostname verification for the master connection (dangerous). |
| `DATABASE_REDIS_MASTER_TLS_CA_PATH` | PEM CA certificate path for the master connection (private CAs). |
| `DATABASE_REDIS_MASTER_TLS_CLIENT_CERT_PATH` | PEM client certificate path for mutual TLS to the master. |
| `DATABASE_REDIS_MASTER_TLS_CLIENT_KEY_PATH` | PEM client private key path for mutual TLS to the master. |
