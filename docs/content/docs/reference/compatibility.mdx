...
| `@ably/ai-transport` | Optional `ably-compat` feature exposing a reduced Ably Pub/Sub subset for AI Transport only; see [Ably AI Transport compatibility](/docs/server/ably-ai-transport-compatibility) |
...
Sockudo does not claim full Ably platform compatibility. The Ably-compatible surface is feature
gated behind `ably-compat` and scoped to AI Transport tests and demos: run lifecycle, mutable
streaming, history projection, and recovery through Sockudo-backed durable primitives.
