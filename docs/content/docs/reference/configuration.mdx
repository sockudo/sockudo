...
| --- | --- | --- | --- |
| `serialization` | `string` | `"json"` | Serialization format for inter-node pub/sub payloads. `"json"` (default, backward compatible) or `"msgpack"` (~45-55% smaller). Receive-side auto-detects format for safe rolling deploys. |
