...
than `analytics_retention_days`, expired idempotency records, expired scheduler locks, and persisted
dead-letter inspection rows where the selected store supports them. Cleanup is bounded by
