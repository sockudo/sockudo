...
Compatibility statistics are bounded in process memory. Compatibility device
registrations and channel subscriptions use `[push].storage_driver`, so clustered
or restart-safe deployments should select the same durable push store used by
native push APIs.
