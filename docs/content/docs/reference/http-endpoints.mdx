...
| Method | Path | Auth | Purpose |
| --- | --- | --- | --- |
| `POST` / `GET` / `DELETE` | `/mcp` (configurable; optionally a dedicated port) | `Authorization: Bearer <mcp token>` | Model Context Protocol Streamable HTTP endpoint. Requires the `mcp` Cargo feature and `[mcp].enabled`. Tools are scope-gated (`read`, `write`, `admin`) and call the routes below in-process. See [MCP server](/docs/server/mcp). |
...
| `POST` | `/apps/{appId}/push/liveActivities/channels` | Signed app API, push admin | Create an APNs Live Activity broadcast channel. |
| `GET` | `/apps/{appId}/push/liveActivities/channels` | Signed app API, push admin | List every APNs broadcast channel for the configured bundle. |
| `GET` | `/apps/{appId}/push/liveActivities/channels/{channelId}` | Signed app API, push admin | Read an APNs broadcast channel and storage policy. |
| `DELETE` | `/apps/{appId}/push/liveActivities/channels/{channelId}` | Signed app API, push admin | Delete an APNs broadcast channel. |
