...
#[tokio::test]
#[ignore = "requires the local audit MySQL fixture on port 13306"]
async fn mysql_compact_versions_counters_imports_and_retention() {
    ...
    let store = MysqlVersionStore::new(
        &config,
        &DatabasePooling {
            enabled: false,
            ..Default::default()
    ...
    let size_sql = format!(
        "SELECT CAST(SUM(payload_size_bytes) AS SIGNED) FROM {}",
...
async fn mysql_metadata_counts_preserve_legacy_and_latest_states() {
    let config = DatabaseConnection {
        host: "127.0.0.1".into(),
        port: 13306,
        username: "root".into(),
