...
pub type DynamoDbAppStore = DynamoDbAppManager;
...
#[async_trait]
impl AppStore for DynamoDbAppManager {
    type Error = Error;
...
pub async fn new_dynamodb_store(config: DynamoDbConfig) -> Result<DynamoDbAppManager> {
    DynamoDbAppManager::new(config).await
}
