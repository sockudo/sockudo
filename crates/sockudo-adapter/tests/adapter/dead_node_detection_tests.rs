...
async fn test_heartbeat_tracking_updates_node_registry() {
    ...
    let heartbeat_request = sockudo_adapter::horizontal_adapter::RequestBody {
    ...
    if !cleanup_tasks.is_empty() {
        let orphaned_members = cleanup_tasks
            ...
            .map(|(app_id, channel, user_id, user_info)| {
                sockudo_adapter::horizontal_adapter::OrphanedMember {
                    app_id,
                    channel,
                    user_id,
                    user_info,
...
async fn test_follower_cleanup_only_removes_registry_data() {
    ...
    let dead_node_request = sockudo_adapter::horizontal_adapter::RequestBody {
