    ...
    pub async fn record_ai_observability(&self, app: &App, channel: &str, message: &PusherMessage) {
        ...
        if let Some(metrics) = self.metrics() {
            ...
            if let Some(stream) = update.stream.as_ref() {
                if stream.active_stream_delta != 0 {
                    metrics.add_ai_active_streams(&app.id, stream.active_stream_delta as i64);
                }
        ...
        if let Some(run_started) = update.run_started.as_ref()
            && let Err(error) = webhook_integration
                ...
                .await
        {
            tracing::warn!(error = %error, "failed to enqueue ai_run_started webhook");
        }
        ...
        if let Some(run_started) = update.run_started.as_ref()
            && let Err(error) = webhook_integration
                ...
                .await
        {
            tracing::warn!(error = %error, "failed to enqueue ai_turn_started webhook");
        }
        ...
        if let Some(run_ended) = update.run_ended.as_ref()
            && let Err(error) = webhook_integration
                ...
                .await
        {
            tracing::warn!(error = %error, "failed to enqueue ai_run_ended webhook");
        }
        ...
        if let Some(run_ended) = update.run_ended.as_ref()
            && let Err(error) = webhook_integration
                ...
                .await
        {
            tracing::warn!(error = %error, "failed to enqueue ai_turn_ended webhook");
        }
        ...
        if let Some(cancel_requested) = update.cancel_requested.as_ref()
            && let Err(error) = webhook_integration
                ...
                .await
        {
            tracing::warn!(error = %error, "failed to enqueue ai_cancel_requested webhook");
        }
    }

    pub fn expire_ai_observability_streams(&self, now_ms: i64, max_idle_ms: i64) {
        let Some(tracker) = self.ai_observability_tracker.as_ref() else {
            return;
        };
        let expired_apps = tracker.expire_stale(now_ms, max_idle_ms);
        if let Some(metrics) = self.metrics() {
            for app_id in expired_apps {
                metrics.add_ai_active_streams(&app_id, -1);
            }
        }
    }
}
