use sockudo_core::utils::{parse_bool_env, parse_env, parse_env_optional};
use ahash::AHashMap;
use serde::{Deserialize, Serialize};
use sockudo_config::adapter::{AdapterConfig, ClusterHealthConfig};
use sockudo_config::drivers::{AppManagerDriver, CacheDriver, MetricsDriver};
use sockudo_config::queue::QueueConfig;
use sockudo_config::webhook::WebhooksConfig;
use std::str::FromStr;
...
fn validate_single_origin_pattern(pattern: &str) -> Result<(), String> {
    if pattern.is_empty() {
    ...
    D: serde::Deserializer<'de>,
{
    ...
    if let Some(ref origins) = value {
        for pattern in origins {
            if let Err(validation_error) = validate_single_origin_pattern(pattern) {
...
pub struct ServerOptions {
    ...
    pub cleanup: sockudo_core::cleanup::CleanupConfig,
...
pub struct ConfiguredApp {
    ...
    #[serde(default)]
    pub webhooks: Option<Vec<sockudo_types::webhook::Webhook>>,
    #[serde(default)]
    ...
    pub channel_delta_compression:
        Option<AHashMap<String, sockudo_types::delta::ChannelDeltaConfig>>,
}
...
impl WebSocketConfig {
    ...
    pub fn to_buffer_config(&self) -> sockudo_types::websocket::WebSocketBufferConfig {
        use sockudo_types::websocket::{BufferLimit, WebSocketBufferConfig};
...
impl Default for ServerOptions {
    fn default() -> Self {
        Self {
            ...
            cleanup: sockudo_core::cleanup::CleanupConfig::default(),
    ...
    pub async fn override_from_env(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Ok(worker_threads_str) = std::env::var("CLEANUP_WORKER_THREADS") {
            self.cleanup.worker_threads = if worker_threads_str.to_lowercase() == "auto" {
                sockudo_core::cleanup::WorkerThreadsConfig::Auto
            } else if let Ok(n) = worker_threads_str.parse::<usize>() {
                sockudo_core::cleanup::WorkerThreadsConfig::Fixed(n)
            } else {
                warn!(
...
impl From<ConfiguredApp> for sockudo_types::app::App {
    fn from(value: ConfiguredApp) -> Self {
        Self {
            id: value.id,
            key: value.key,
            secret: value.secret,
