...
mod tests {
    ...
    async fn test_validate_channel_name() {
        let max_channel_name_length = Some(50);

        // Test valid channel names
        assert!(
            validate_channel_name(max_channel_name_length, "test-123")
                .await
                .is_ok()
        );
        assert!(
            validate_channel_name(max_channel_name_length, "user@domain")
                .await
                .is_ok()
        );
        assert!(
            validate_channel_name(max_channel_name_length, "channel.name")
                .await
                .is_ok()
        );
        assert!(
            validate_channel_name(max_channel_name_length, "#server-to-user-123")
                .await
                .is_ok()
        );
        ...
        assert!(
            validate_channel_name(max_channel_name_length, "invalid$channel")
                .await
                .is_err()
        );
        assert!(
        ...
        assert!(
            validate_channel_name(max_channel_name_length, "space in name")
                .await
                .is_err()
        );
    }
