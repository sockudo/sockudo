...
use sockudo_types::app::App; // To access app limits
...
async fn process_single_event_parallel(
    ...
    let channel_processing_futures = target_channels.into_iter().map(|target_channel_str| {
        ...
        async move {
            ...
            validate_channel_name(app.max_channel_name_length, &target_channel_str).await?;
...
pub async fn channel(
    ...
    validate_channel_name(app.max_channel_name_length, &channel_name).await?;
...
pub async fn channels(
    ...
    for (channel_name_str, _socket_count) in &channels_map {
        ...
        validate_channel_name(app.max_channel_name_length, channel_name_str).await?;
        let mut current_channel_info_map = sonic_rs::Object::new();
        if wants_user_count {
...
async fn check_system_health(handler: &Arc<ConnectionHandler>) -> HealthStatus {
    ...
    if handler.server_options().cache.driver != CacheDriver::None {
