...
use sockudo_queue_redis_cluster::QueueBackend as _;
use sockudo_queue_redis_cluster::backend::RedisClusterQueueManager as RedisClusterQueueBackend;
use sockudo_types::webhook::JobData;

pub struct RedisClusterQueueManager {
    inner: RedisClusterQueueBackend,
}
...
impl RedisClusterQueueManager {
    pub async fn new(
        ...
        let inner = RedisClusterQueueBackend::new(cluster_nodes, prefix, concurrency)
            .await
...
impl QueueInterface for RedisClusterQueueManager {
    ...
    async fn process_queue(
        ...
        let adapted_callback: sockudo_queue_redis_cluster::JobProcessorFnAsync =
            Box::new(move |job| {
