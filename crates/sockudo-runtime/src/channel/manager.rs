...
use sockudo_core::channel_store::ChannelStore;
...
impl ChannelManager {
    ...
    where
        C: ChannelStore<Error = Error> + Send + Sync + ?Sized,
    {
    ...
    pub async fn get_channel_members<C>(
        connection_manager: &Arc<C>,
        app_id: &str,
        channel: &str,
    ...
    pub async fn batch_unsubscribe<C>(
        connection_manager: &Arc<C>,
