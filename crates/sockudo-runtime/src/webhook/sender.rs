...
use ahash::AHashMap;
use reqwest::Client;
use sockudo_types::webhook::{JobData, PusherWebhookPayload, Webhook};
use sonic_rs::Value;
#[cfg(feature = "lambda")]
...
async fn send_pusher_webhook(
    ...
    let http_sender = sockudo_webhook_http::HttpWebhookSender::new();
    http_sender
        .send_pusher_webhook(url, app_key, signature, json_body, custom_headers_config)
        .await
        .map_err(Error::Other)
}
