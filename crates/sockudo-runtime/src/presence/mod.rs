...
use dashmap::DashMap;
use sockudo_core::connection_lifecycle::ConnectionLifecycleStore;
...
impl PresenceManager {
    ...
    pub async fn handle_member_added<C>(
        &self,
    ...
    where
        C: ConnectionLifecycleStore<Error = crate::error::Error, Message = PusherMessage>
            + Send
            + Sync
            + ?Sized,
    {
    ...
    ///
    /// FIX: Uses atomic check-and-act pattern to prevent TOCTOU race conditions
    pub async fn handle_member_removed<C>(
        &self,
        connection_manager: &Arc<C>,
        webhook_integration: Option<&Arc<WebhookIntegration>>,
        app_config: &App,
    ...
    async fn user_has_other_connections_in_presence_channel<C>(
        connection_manager: Arc<C>,
        app_id: &str,
        channel: &str,
        user_id: &str,
    ...
    async fn broadcast_to_channel<C>(
        connection_manager: Arc<C>,
        app_id: &str,
        channel: &str,
