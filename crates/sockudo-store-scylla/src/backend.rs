...
pub type ScyllaDbAppStore = ScyllaDbAppManager;
...
#[async_trait]
impl AppStore for ScyllaDbAppManager {
    type Error = Error;
...
pub async fn new_scylla_store(config: ScyllaDbConfig) -> Result<ScyllaDbAppManager> {
    ScyllaDbAppManager::new(config).await
}
