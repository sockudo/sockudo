...
impl RedisClusterCacheManager {
    ...
    pub async fn new(config: RedisClusterCacheConfig) -> Result<Self> {
        ...
        let connection = client
            ...
            .await
            .map_err(|e| Error::CacheError(format!("Failed to connect to Redis Cluster: {}", e)))?;
    ...
    async fn set(&mut self, key: &str, value: &str, ttl_seconds: u64) -> Result<()> {
        ...
        if ttl_seconds > 0 {
            ...
            self.connection
                ...
                .await
                .map_err(|e| Error::CacheError(format!("Redis Cluster set error: {}", e)))?;
        } else {
...
impl RedisClusterCacheManager {
    ...
    pub async fn clear_prefix(&mut self) -> Result<usize> {
        ...
        let keys: Vec<String> = redis::cmd("KEYS")
            ...
            .await
            .map_err(|e| Error::CacheError(format!("Redis Cluster keys error: {}", e)))?;
        ...
        for (key, value) in &prefixed_pairs {
            if ttl_seconds > 0 {
                self.connection
                    ...
                    .await
                    .map_err(|e| Error::CacheError(format!("Redis Cluster set_ex error: {}", e)))?;
            } else {
                self.connection
                    .set::<_, _, ()>(key, *value)
                    .await
    ...
    pub async fn get_cluster_info(&mut self) -> Result<String> {
        let info: String = redis::cmd("CLUSTER")
            ...
            .query_async(&mut self.connection)
            .await
            .map_err(|e| Error::CacheError(format!("Redis Cluster info error: {}", e)))?;
    ...
    pub async fn get_cluster_nodes(&mut self) -> Result<String> {
        let nodes: String = redis::cmd("CLUSTER")
            ...
            .query_async(&mut self.connection)
            .await
            .map_err(|e| Error::CacheError(format!("Redis Cluster nodes error: {}", e)))?;
