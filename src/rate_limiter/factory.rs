...
use crate::options::{RateLimiterConfig, RedisConfig as RateLimiterRedisBackendConfig, CacheDriver, RedisConnection};
...
impl RateLimiterFactory {
    pub async fn create(
        ...
        if !config.enabled {
            Log::info("HTTP API Rate limiting is globally disabled. Returning a permissive limiter.".to_string());
            return Ok(Arc::new(MemoryRateLimiter::new(u32::MAX, 1))); // Allows all
        }

        Log::info(format!("Initializing HTTP API RateLimiter with driver: {:?}", config.driver));
        match config.driver {
            CacheDriver::Redis => { // Assuming RateLimiter uses CacheDriver enum for its backend
                ...
                if config.redis.cluster_mode {
                    ...
                    if global_redis_conn_details.cluster_nodes.is_empty() {
                        ...
                        return Err(crate::error::Error::ConfigurationError("RateLimiter: Redis cluster nodes not configured.".to_string()));
                    }
                    ...
                    let client = redis::Client::open(redis_url.as_str())
                        .map_err(|e| crate::error::Error::RedisError(format!("Failed to create Redis client for rate limiter: {}", e)))?;
            ...
            CacheDriver::RedisCluster => { // Explicit RedisCluster driver for RateLimiter backend
                Log::info("RateLimiter: Using Redis Cluster backend (explicitly selected).".to_string());
                if global_redis_conn_details.cluster_nodes.is_empty() {
            ...
            CacheDriver::Memory | _ => { // Default to memory for rate limiter if driver is "memory" or unknown
