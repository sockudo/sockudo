...
impl AppManagerFactory {
    pub async fn create(
        ...
        Log::info(format!("Initializing AppManager with driver: {:?}", config.driver));
        match config.driver { // Match on the enum
            AppManagerDriver::Mysql => {
                let mysql_db_config = db_config.mysql.clone();
                match MySQLAppManager::new(mysql_db_config.into()).await {
            ...
            AppManagerDriver::Dynamodb => {
                let dynamo_app_config = DynamoDbConfig {
                    region: db_config.redis.host.clone(), // Placeholder, adjust region source
                    table_name: db_config.mysql.table_name.clone(), // Placeholder, adjust table name source
                    endpoint: None,
                    access_key: None,
                    secret_key: None,
            ...
            AppManagerDriver::Memory | _ => { // Handle unknown as Memory or make it an error
