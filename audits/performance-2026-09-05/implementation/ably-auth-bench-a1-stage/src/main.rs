...
async fn main(){
 ...
 eprintln!("fixture_prefix={prefix}");
 if std::env::var_os("PAGE_PROBE").is_some() {
  // Match the two earlier 128-record apps in the complete benchmark cohort,
  // preserving global SCAN density and filter selectivity rather than only matches.
  if std::env::var_os("MATCH_COHORT_DENSITY").is_some() {
   for app in ["app-128-100","app-128-1000"] { for index in 0..128 {
    let target=format!("other-{index}");
    let record=serde_json::to_string(&AblyRevocationRecord{target_type:"clientId".into(),target_value:target.clone(),issued_before:now_ms()-999,applies_at:now_ms()-1000}).unwrap();
    cache.set(&revocation_cache_key(app,"clientId",&target),&record,3600).await.unwrap();
   }}
  }
  for index in 0..1024 {
   let target=format!("other-{index}");
   let record=serde_json::to_string(&AblyRevocationRecord{target_type:"clientId".into(),target_value:target.clone(),issued_before:now_ms()-999,applies_at:now_ms()-1000}).unwrap();
   cache.set(&revocation_cache_key("app-1024-100","clientId",&target),&record,3600).await.unwrap();
  }
  for rep in 0..2 {
   let mut cursor=None;let mut count=0;let mut pages=0;let mut max_page_us=0;let started=Instant::now();
   loop {
    let one=Instant::now();let page=cache.scan_prefix_page("ably-compat:revocation:app-1024-100:",cursor,512).await.unwrap();max_page_us=max_page_us.max(one.elapsed().as_micros());count+=page.entries.len();pages+=1;
    cursor=page.next_cursor;if cursor.is_none(){break;}
   }
   assert_eq!(count,1024);println!("a1page,rep={rep},records={count},pages={pages},us={},max_page_us={max_page_us}",started.elapsed().as_micros());
  }
  owned.0.clear_prefix().await.unwrap();return;
 }
