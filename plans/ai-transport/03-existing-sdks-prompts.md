...
>
> **The ecosystem (in-tree, audited 2026-06-02 — absent from CLAUDE.md):**
>
> | Kind | SDKs |
> |---|---|
> | Client (realtime) | `client-sdks/sockudo-js` (**`@sockudo/client`** v1.3+, with react / vue / react-native / nativescript / worker subpaths), `sockudo-dotnet`, `sockudo-flutter`, `sockudo-kotlin`, `sockudo-swift` |
...
>
> `sockudo-js` already implements V2 resume, rewind (+`rewind_complete`), a `channel_history` action, and mutable-message reducers (`reduceMutableMessageEvent(s)`). Several others likely implement subsets — **E0 establishes the truth.**
...
E4a–E4d (dotnet/flutter/kotlin/swift upgrades) ←──────────────────────────────────────┤
...
server (crates/) AND the full SDK ecosystem in-tree: client-sdks/{sockudo-js, sockudo-dotnet,
sockudo-flutter, sockudo-kotlin, sockudo-swift} and server-sdks/sockudo-http-{node, php, python,
ruby, go, java, dotnet, rust, swift}. Each SDK directory is its own git repo/submodule with its
own toolchain, CI, and conventions — detect and respect them (read its README/CI config first).
...
## 4. Common Upgrade Spec (referenced by E4a–E4d — paste along with those prompts)
...
SCOPE
1. INVENTORY — for each of the 5 client SDKs and 9 server SDKs: name, package id + published
   version, language/toolchain, repo/submodule status (branch, last commit, CI setup), exact
...
3. FORWARD-COMPAT FIXTURES (test/ai-conformance/fixtures/forward-compat/): golden frames
   ...
   added in E2/E3). Single source of truth; SDK repos consume via path/submodule, never copy.
...
**Depends on:** E0 (risk register), E1 (fixtures). One PR per SDK; this prompt is run once per client SDK — `sockudo-js`, `sockudo-dotnet`, `sockudo-flutter`, `sockudo-kotlin`, `sockudo-swift` — substituting the SDK name.
...
2. Wire the forward-compat fixtures (E1 §3) into this SDK's unit suite via a small replay
   ...
   and maintained in this SDK's repo; fixtures come from the server repo (path dep/submodule —
...
### E4a–E4d — Client SDK platform-primitive upgrades (enablement)
...
> **Note:** the `sockudo-js` upgrade is **file 02, prompt P1** — already specified there; do not duplicate. These four bring the rest of the client fleet to parity. Run in any order after sockudo-js.
